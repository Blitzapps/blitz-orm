import { action, createMachine, guard, immediate, interpret, invoke, state, transition, reduce } from 'robot3';
import { BQLMutationBlock, EnrichedBQLMutationBlock, EnrichedBormSchema } from '../../types';
import { enrichBQLMutation } from './BQL/enrich';
import { isArray } from 'radash';
import { getCurrentSchema } from '../../helpers';
import { TqlMutation, runTQLMutation } from './TQL/run';
import { TqlRes, parseTQLMutation } from './TQL/parse';
...
export const machine = createMachine(
  'enrich',
  {
    enrich: invoke(
      enrich,
      transition('done', 'addIntermediaries', reduce(updateBqlReq)),
      errorTransition,
    ),
    ...
    parseBQL: invoke(
      parseBQL,
      transition('done', 'buildMutation', reduce(updateThingsEdges)),
      errorTransition,
    ),
