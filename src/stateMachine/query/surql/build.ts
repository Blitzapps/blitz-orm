import type { EnrichedAttributeQuery, EnrichedBQLQuery, EnrichedBormEntity, EnrichedBormRelation, EnrichedBormSchema, EnrichedEntityQuery, EnrichedFieldQuery, EnrichedLinkField, EnrichedLinkQuery, EnrichedRoleField, EnrichedRoleQuery, Filter, PositiveFilter } from '../../../types';
...
const buildQuery = (props: {
  ...
  const fieldLines = buildFieldsQuery({ queries: query.$fields, level: 1, schema, thingSchema });
  if (fieldLines) {
...
const buildFieldsQuery = (props: {
  ...
  const { queries, schema, thingSchema, level } = props;
  const lines: string[] = [];
  queries.forEach((i) => {
    const line = buildFieldQuery({ query: i, level, schema, thingSchema });
    if (line) {
      lines.push(line);
    }
  });
...
const buildLinkQuery = (props: {
  ...
  const fieldLines = buildFieldsQuery({ queries: query.$fields, level: fieldLevel, schema, thingSchema });
  if (fieldLines) {
    lines.push(fieldLines);
  }
  ...
  if (query.$target === 'relation') {
    ...
    from = things.map((thing) => `<-\`${query.$playedBy.relation}_${query.$plays}\`<-\`${query.$playedBy.relation}\`->\`${query.$playedBy.relation}_${query.$playedBy.plays}\`->\`${thing}\``).join(', ');
  }
