...
until docker exec $CONTAINER_NAME ./surreal is-ready --endpoint http://localhost:8001 2>/dev/null; do
    sleep 0.5;
done;
echo "SurrealDB is ready!"

# Setup surrealdb database: create the namespace, database, and user dynamically
docker exec -i $CONTAINER_NAME ./surreal sql -u $USER -p $PASSWORD --endpoint http://localhost:8001 <<EOF
